...
def get_official_eval_result_OLDversion(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
...
def get_official_eval_result(gt_annos, dt_annos, current_classes, PR_detail_dict=None):
    ...
    mAPbbox, mAPbev, mAP3d, mAPaos, mAPbbox_R40, mAPbev_R40, mAP3d_R40, mAPaos_R40 = do_eval(
        gt_annos, dt_annos, current_classes, min_overlaps, compute_aos, PR_detail_dict=PR_detail_dict)
    if not compute_aos:
        mAPaos = mAPaos_R40 = np.zeros_like(mAPbbox)
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if i == 1:  # 0, or 1
                ...
                #     class_to_name[curcls], 'easy', 'moderate', 'hard',
                #     'bbox_R11', mAPbbox[j, 0, i], mAPbbox[j, 1, i], mAPbbox[j, 2, i],
                #     'bev_R11', mAPbev[j, 0, i], mAPbev[j, 1, i], mAPbev[j, 2, i],
                #     '3d_R11', mAP3d[j, 0, i], mAP3d[j, 1, i], mAP3d[j, 2, i],
                #     'aos_R11', mAPaos[j, 0, i], mAPaos[j, 1, i], mAPaos[j, 2, i],
                # )
                table = "\n{:<12}{:<12}{:<12}{:<12}\n" \
                        "{:<12}{:<12.2f}{:<12.2f}{:<12.2f}\n" \
                        "{:<12}{:<12.2f}{:<12.2f}{:<12.2f}\n" \
                        ...
                        "{:<12}{:<12.2f}{:<12.2f}{:<12.2f}\n".format(
                    class_to_name[curcls], 'easy', 'moderate', 'hard',
                    'bbox_R40', mAPbbox_R40[j, 0, i], mAPbbox_R40[j, 1, i], mAPbbox_R40[j, 2, i],
                    'bev_R40', mAPbev_R40[j, 0, i], mAPbev_R40[j, 1, i], mAPbev_R40[j, 2, i],
                    '3d_R40', mAP3d_R40[j, 0, i], mAP3d_R40[j, 1, i], mAP3d_R40[j, 2, i],
                    'aos_R40', mAPaos_R40[j, 0, i], mAPaos_R40[j, 1, i], mAPaos_R40[j, 2, i],
                )
                print(table)
...
def store_and_print_result(gt_dir, dt_dir, cur_classes):
    gt_dict = get_label_annos(gt_dir)
    dt_dict = get_label_annos(dt_dir + r'/data')
    print(f"Got {len(os.listdir(dt_dir+r'/data'))} files for evaluation ...")
    result = get_official_eval_result(gt_dict, dt_dict, cur_classes)

    if os.path.exists(dt_dir + r'/result.txt'):
        print(f'Old File exists: {dt_dir}/result.txt')
        return print(result)
    file = open(dt_dir + r'/result.txt', 'w')
    file.write(result)
    file.close()
...
if __name__ == "__main__":
    ...
    pred_file = '/home/dell/PaperCode/MyDetection/DS-LV-GGNN/preds/kitti/Car/test79'
